...
export function useGenerationSocket(events?: GenerationEvents): UseGenerationSocketReturn {
  ...
  const createConnection = useCallback(() => {
    ...
    if (!tokenService.isValidTokenFormat(accessToken)) {
      console.error('🔐 Invalid token format, cannot establish WebSocket connection');
      return null;
    }
  ...
  const scheduleReconnect = useCallback(() => {
    if (connectionAttempts >= maxRetries) {
      ...
      eventsRef.current?.onError?.('Connection failed after maximum retry attempts');
      return;
    }
  ...
  const startHealthCheck = useCallback((socketInstance: Socket) => {
    healthCheckIntervalRef.current = setInterval(() => {
      if (socketInstance.connected) {
        socketInstance.emit('ping', { timestamp: Date.now() });
      } else {
        console.warn('⚠️ Socket disconnected during health check, attempting reconnection');
        scheduleReconnect();
      }
  ...
  const initializeConnection = useCallback(() => {
    ...
    socketInstance.on('error', (error) => {
      ...
      eventsRef.current?.onError?.(typeof error === 'string' ? error : 'Socket error occurred');
    });

    // Health check response handler
    socketInstance.on('pong', (data) => {
      const latency = Date.now() - data.timestamp;
      if (latency > 5000) { // High latency warning
